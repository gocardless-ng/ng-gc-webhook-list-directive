...
angular.module('webhook-list-template.html', []).run(['$templateCache', function($templateCache) {
  $templateCache.put('webhook-list-template.html',
    '<div><div class="well welcome-annotation" ng-hide="webHooks"><figure class="icon-welcome-arrow welcome-annotation__arrow welcome-annotation__arrow--payment"></figure><div class="page-notification"><figure class="page-notification-icon page-notification-icon--large ss-connection"></figure><h1 class="u-text-heading">Send your first web hook</h1><p>Web hooks will appear here, but none have been triggered yet.<br>Start by sending a new web hook.</p></div></div><!-- BODY CELL TEMPLATES --><span gc-set-fragment="tableBody:created_at" class="u-text-truncate u-block">{{ $row.created_at | date }}</span> <span gc-set-fragment="tableBody:response_code" class="u-text-truncate u-block"><span class="status-code" ng-class="\'status-code--\' + getStatusCodeLabel($row.response_code)">{{ $row.response_code || \'-\' }}</span></span> <!-- BODY CELL TEMPLATES END --><gc-table table-data="webHooks" table-options="{\n' +
    ...
    '    }]"></gc-table><pagination-nav meta="webHooks.meta"></pagination-nav></div>');
}]);
